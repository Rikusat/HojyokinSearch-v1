...
# Create tabs for each unique 対象事業者
tabs = st.tab(unique_対象事業者)

# Show the results and balloons, if you have a text_search
if text_search:
    for tab in tabs:
        # Filter the dataframe for the current tab
        df_tab = df_search[df_search["対象事業者"] == tab]
        ...
        # Show the cards
        N_cards_per_row = 3
        for n_row, row in df_tab.reset_index().iterrows():
